#ifndef ICE2004_WEEK13_HW01_HELPER_H
#define ICE2004_WEEK13_HW01_HELPER_H
...
// `std::pair`에 대해 템플릿 특수화된 코드입니다.
template<typename T, typename U>
void print_array(const std::pair<T, U>* array, std::size_t size) {
    for (std::size_t i = 0; i < size; i += 1) {
        std::cout << array[i].first << '(' << array[i].second << ')' << ' ';
    }
}

#endif // ICE2004_WEEK13_HW01_HELPER_H
